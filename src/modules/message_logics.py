...
async def unknown_command_handler(update: Update) -> None:
    await update.message.reply_text(
        DIC["unknown_command"].format(command=update.message.text)
    )


async def plain_text_handler(update: Update) -> None:
...
async def photo_handler(update: Update) -> None:
    await update.message.reply_text(DIC["photo"])


async def audio_handler(update: Update) -> None:
    await update.message.reply_text(DIC["audio"])


async def voice_handler(update: Update) -> None:
    await update.message.reply_text(DIC["voice"])


async def video_handler(update: Update) -> None:
    await update.message.reply_text(DIC["video"])


async def document_handler(update: Update) -> None:
    await update.message.reply_text(DIC["document"])


async def sticker_handler(update: Update) -> None:
    await update.message.reply_text(DIC["sticker"])


async def location_handler(update: Update) -> None:
    await update.message.reply_text(DIC["location"])


async def contact_handler(update: Update) -> None:
